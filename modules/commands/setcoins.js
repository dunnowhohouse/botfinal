...
module.exports.run = async function({ api, Users, Threads, event, Currencies, global, args
  ...
  if (!args[0]) return api.sendMessage("⚡️Sai cú pháp. setmoney id/tag/me/reset", event.threadID, event.messageID);
  var { body } = event;
 ...
 ///api.sendMessage(moneySet, event.threadID);
  if (isNaN(moneySet))
    return api.sendMessage("⚡️Số tiền cần set phải là một con số", event.threadID, event.messageID);

    switch (args[0]){
    case "id":{
      if (!args[2]) return api.sendMessage("⚡️sai cú pháp. $setmoney id [id] [money]", event.threadID, event.messageID);
    var idu = args[1];
    var ten = (await Users.getData(idu)).name;

    return api.sendMessage(
      `⚡️Đã cộng ${moneySet} coin vào tài khoản của ${ten}`,
      event.threadID,
      async () => {
  ...
  case "me": {
api.sendMessage(
      `⚡️Đã cộng ${moneySet} đô vào tài khoản của ${
  ...
  return api.sendMessage(`⚡️Đã reset toàn bộ số tiền của user ${name1}`, event.threadID, async () => {
...
break;
  ...
  if (!idd) return; 
    ...
    return api.sendMessage(
      `⚡️Đã cộng ${moneySet} vào tài khoản của ${name}`,
      event.threadID,
      async () => {
