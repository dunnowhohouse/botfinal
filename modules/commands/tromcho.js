...
async function makeImage({ one, two }) {
    ...
    tromcho_img.composite(circleOne.resize(50, 50), 234, 38).composite(circleTwo.resize(90, 90), 50, 234);
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "M coi chừng t" + tag + 'xích m lại nhé sủa cl! 😈😈',
            mentions: [{
          tag: tag,
          id: mention
        }],
